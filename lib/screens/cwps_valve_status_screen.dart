// lib/screens/cwps_valve_status_screen.dart
import 'dart:convert';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;

class CwpsValveStatusScreen extends StatefulWidget {
  const CwpsValveStatusScreen({super.key});

  @override
  State<CwpsValveStatusScreen> createState() => _CwpsValveStatusScreenState();
}
...
class _CwpsValveStatusScreenState extends State<CwpsValveStatusScreen> {
  ...
  static const List<String> _valveKeys = ['A7','A8','A9','A10','A11','A12','A13'];

  // TAB3: A1..A7 -> Valve Position 1..7 (%)
  static const List<String> _positionKeys = ['A1','A2','A3','A4','A5','A6','A7'];

   String _vTitle(int idx) => idx == 7 ? 'Valve 7 (Common)' : 'Valve $idx (Pump $idx)';
   String _vpTitle(int idx) => idx == 7 ? 'Valve Position 7 (Common)' : 'Valve Position $idx (Pump $idx)';
  ...
  Future<_ValveFetchResult> _fetch() async {
    ...
    // Valves (OPEN/CLOSE) from TAB1 A7..A13
  ...
  Widget build(BuildContext context) {
    final String title = _showPositions ? 'CWPS • Valve Positions' : 'CWPS • Valves';
...
class _ValveTileStack extends StatelessWidget {
  ...
  final bool isOpen;
  const _ValveTileStack({super.key, required this.title, required this.stateText, required this.isOpen});
