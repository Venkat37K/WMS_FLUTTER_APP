...
class CwpsAnalyzerScreen extends StatefulWidget {
  const CwpsAnalyzerScreen({super.key});

  @override
  State<CwpsAnalyzerScreen> createState() => _CwpsAnalyzerScreenState();
}
...
class _CwpsAnalyzerScreenState extends State<CwpsAnalyzerScreen> {
  ...
  static const List<_ParamDef> _params = [
    _ParamDef(label: 'PH',                            key: 'A11', unit: 'pH'),
    _ParamDef(label: 'Conductivity',                  key: 'A12', unit: 'µS/m'),
    _ParamDef(label: 'Oxidation Reduction Potential', key: 'A13', unit: 'mV'),
    _ParamDef(label: 'Free chlorine',                 key: 'A14', unit: 'mg/L'),
    _ParamDef(label: 'Total chlorine',                key: 'A15', unit: 'mg/L'),
  ];
  ...
  Future<_AnalyzerFetchResult> _fetchAnalyzer() async {
    ...
    final tab2 = (t2Any is Map<String, dynamic>) ? t2Any : <String, dynamic>{};
  ...
  static String _formatValue(dynamic raw) {
    if (raw == null) return '--';
    if (raw is num) {
      // Integers as-is; doubles to 2 decimals (hide trailing .0)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('CWPS • Analyzer', style: TextStyle(color: Colors.white)),
