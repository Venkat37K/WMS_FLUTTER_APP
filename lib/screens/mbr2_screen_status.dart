...
class Mbr2StatusScreen extends StatefulWidget {
  const Mbr2StatusScreen({super.key});

  @override
  State<Mbr2StatusScreen> createState() => _Mbr2StatusScreenState();
}

class _Mbr2StatusScreenState extends State<Mbr2StatusScreen> {
  // Same endpoint as MBR1
  static const String apiUrl = 'http://192.168.123.154:5000/api/mbr-analog/latest';

  // ---- SAME display settings as MBR1/MSR1 ----
  ...
  Future<_MbrFetchResult> _fetch() async {
    ...
    // Pick the MBR_02 block (tolerant to spacing)
    final mbr02 = (data['MBR_02'] ??
        data['MBR 02'] ??
        data['MBR2'] ??
        {}) as Map<String, dynamic>;
    final sensors = (mbr02['sensors'] ?? {}) as Map<String, dynamic>;
    ...
    // ---- Mapping for MBR_02 ----
    final lt = _toDouble(sensors['ULT 2']);
    final pt = _toDouble(sensors['PT 2']);
    final ft = _toDouble(sensors['FT 2']);
    final ftTotal = _toDouble(sensors['FT_TT']);
    final oclr = _toDouble(sensors['OCLR']);
    final ocnd = _toDouble(sensors['OCND']);

    // Timestamp
    final date = (mbr02['date'] as String?)?.trim();
    final time = (mbr02['time'] as String?)?.trim();
    String? iso;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        // Keep same UI/title pattern
        title: const Text('MBR • MBR 2 - Bharathi Park', style: TextStyle(color: Colors.white)),
          ...
          child: Padding(
            ...
            child: FutureBuilder<_MbrFetchResult>(
              ...
              builder: (context, snap) {
                ...
                if (snap.hasError) {
                  return _ErrorView(
                    message: 'Failed to load MBR 2 data',
...
// ===== model & shared widgets (same as MBR1) =====
