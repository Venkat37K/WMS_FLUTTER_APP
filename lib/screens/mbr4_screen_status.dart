...
class Mbr4StatusScreen extends StatefulWidget {
  const Mbr4StatusScreen({super.key});

  @override
  State<Mbr4StatusScreen> createState() => _Mbr4StatusScreenState();
}

class _Mbr4StatusScreenState extends State<Mbr4StatusScreen> {
  static const String apiUrl = 'http://192.168.123.154:5000/api/mbr-analog/latest';

  // ---- SAME display settings as MBR1/2/3 ----
  ...
  Future<_MbrFetchResult> _fetch() async {
    ...
    // Pick the MBR_04 block (tolerant to variants)
    final mbr04 = (data['MBR_04'] ??
        data['MBR 04'] ??
        data['MBR4'] ??
        {}) as Map<String, dynamic>;
    final sensors = (mbr04['sensors'] ?? {}) as Map<String, dynamic>;
    ...
    // ---- Mapping for MBR 4 ----
    final lt = _toDouble(sensors['ULT 4']);
    final pt = _toDouble(sensors['PT 4']);
    final ft = _toDouble(sensors['FT 4']);
    final ftTotal = _toDouble(sensors['FT_TT']);
    final oclr = _toDouble(sensors['OCLR']);
    final ocnd = _toDouble(sensors['OCND']);

    // Timestamp
    final date = (mbr04['date'] as String?)?.trim();
    final time = (mbr04['time'] as String?)?.trim();
    String? iso;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        // Keep same style; update label for MBR 4
        title: const Text('MBR • MBR 4 - Pillaiyarpuram', style: TextStyle(color: Colors.white)),
          ...
          child: Padding(
            ...
            child: FutureBuilder<_MbrFetchResult>(
              ...
              builder: (context, snap) {
                ...
                if (snap.hasError) {
                  return _ErrorView(
                    message: 'Failed to load MBR 4 data',
...
// ===== shared widgets (identical to other MBR screens) =====
