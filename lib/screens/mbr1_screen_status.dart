// lib/screens/mbr1_screen_status.dart
import 'dart:convert';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;

class Mbr1StatusScreen extends StatefulWidget {
  const Mbr1StatusScreen({super.key});

  @override
  State<Mbr1StatusScreen> createState() => _Mbr1StatusScreenState();
}
...
class _Mbr1StatusScreenState extends State<Mbr1StatusScreen> {
  ...
  Future<_MbrFetchResult> _fetch() async {
    ...
    // Pick the MBR_01 block (be tolerant to spacing)
    final mbr01 = (data['MBR_01'] ??
        data['MBR 01'] ??
        data['MBR1'] ??
        {}) as Map<String, dynamic>;
    final sensors = (mbr01['sensors'] ?? {}) as Map<String, dynamic>;
    ...
    // ---- Mapping (differs from MSR1 only by block name) ----
    final lt1 = _toDouble(sensors['ULT 1']);
    final pt1 = _toDouble(sensors['PT 1']);
    final ft1 = _toDouble(sensors['FT 1']);
    final ftTotal = _toDouble(sensors['FT_TT']);
    final oclr = _toDouble(sensors['OCLR']);
    final ocnd = _toDouble(sensors['OCND']);

    // Timestamp
    final date = (mbr01['date'] as String?)?.trim();
    final time = (mbr01['time'] as String?)?.trim();
    String? iso;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        // Keep title style the same; label for MBR 1
        title: const Text('MBR • MBR 1 - Valarmathi Nagar', style: TextStyle(color: Colors.white)),
          ...
          child: Padding(
            ...
            child: FutureBuilder<_MbrFetchResult>(
              ...
              builder: (context, snap) {
                ...
                if (snap.hasError) {
                  return _ErrorView(
                    message: 'Failed to load MBR 1 data',
...
// ===== shared widgets (identical to MSR1 for consistent UI) =====
