...
class Msr2StatusScreen extends StatefulWidget {
  const Msr2StatusScreen({super.key});

  @override
  State<Msr2StatusScreen> createState() => _Msr2StatusScreenState();
}
...
class _Msr2StatusScreenState extends State<Msr2StatusScreen> {
  ...
  Future<_MsrFetchResult> _fetch() async {
    ...
    // ------- ONLY DIFFERENCE: pick MSR_02 block -------
    final msr02 = (data['MSR_02'] ??
        data['MSR 02'] ??
        data['MSR2'] ??
        {}) as Map<String, dynamic>;

    final sensors = (msr02['sensors'] ?? {}) as Map<String, dynamic>;
    ...
    final lt = _toDouble(sensors['ULT 2']);   // Level Transmitter 2
    final pt = _toDouble(sensors['PT 2']);    // Pressure Transmitter 2
    final ft = _toDouble(sensors['FT 2']);    // Flow Transmitter 2
    ...
    // date & time -> ISO-ish for the "Last update" label
    final date = (msr02['date'] as String?)?.trim();
    final time = (msr02['time'] as String?)?.trim();
    String? iso;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        // Title mirrors MSR1 but says MSR 2
        title: const Text('MSR • MSR 2 - Ramakrishnapuram (New)', style: TextStyle(color: Colors.white)),
          ...
          child: Padding(
            ...
            child: FutureBuilder<_MsrFetchResult>(
              ...
              builder: (context, snap) {
                ...
                if (snap.hasError) {
                  return _ErrorView(
                    message: 'Failed to load MSR 2 data',
                ...
                return RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      // Analyzer (Chlorine & Conductivity)
                      const SizedBox(height: 16),
...
// ===== shared widgets (copied from MSR1 for identical UI) =====
